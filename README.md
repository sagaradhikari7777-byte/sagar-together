...
Canonical source: https://github.com/sagaradhikari7777-byte/sagar-together.

Netlify is linked to this repository. Push updates to `main` to trigger a production build. Build settings are defined in the root `netlify.toml`: `npm run build`, output `dist`, and functions `netlify/functions`. Confirm the matching commit succeeds in Netlify before treating an update as live. Never commit credentials or household data.
