...
test('insights include every expense and distinguish split allocations',()=>{
 const s=summarizeSpending(items);assert.equal(s.total,6001);assert.equal(s.equalSplit,3001);assert.equal(s.coupleAssigned,3000);assert.equal(s.average,2000);assert.deepEqual(s.people,[1001,3000,2000,0]);assert.deepEqual(s.categories,[['Groceries',4001],['Food',2000]]);
